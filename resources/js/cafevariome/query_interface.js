...
$( function() {
    ...
    var handle_meds = $( "#sr-handle-meds" );

    $( "#similarity-range-meds" ).slider({
        range: "min",
        min: 0,
        max: 0,
        value: 1,
        ...
        create: function() {
            if ($(this).slider("value") != 0) {
                handle_meds.text($(this).slider("value"));
            }
        },
        slide: function( event, ui ) {
            handle_meds.text( ui.value );
        }
    });

    $( "#similarity-range-meds" ).slider('disable');

    var handle_maybe = $( "#sr-handle-maybe" );
    $( "#similarity-range-maybe" ).slider({
        range: "min",
        min: 0,
        max: 0,
        value: 1,
        ...
        create: function() {
            if ($(this).slider("value") != 0) {
                handle_maybe.text($(this).slider("value"));
            }
        },
        slide: function( event, ui ) {
            handle_maybe.text( ui.value );
        }
    });
    $( "#similarity-range-maybe" ).slider('disable');
...
console.log("DONE!"+jsonData);
    ...
    $('#search_med_phen_left').keyup(function() {
        if($search_str2 == $(this).val()) return;
        $('select#med_values_phen_left').empty()
        var arrayToReduce = $(this).val().trim().split(' ').filter((term) => term.length != 1);
        str = (arrayToReduce.length > 0) ? arrayToReduce.reduce((v1, v2) => v1 + " " + v2) : '';

        $.ajax({
            url: 'https://autocomplete.cafevariome.org/Snomed/medication/' + str+'/1',
            type: 'GET',
            dataType:'json',
            crossDomain: true,
            success: function  (data, textStatus, jqXHR) {
                $('select#med_values_phen_left').empty()
                data.forEach((term) => {
                    $('select#med_values_phen_left').append($('<option></option>').attr('value', term).text(term))
                })
            }
        }).done(function() {
    ...
    $('button.btnRemove2').click(() => {
        $("#similarity-range-meds").slider('disable');
        var max_val_meds = $("#similarity-range-meds").slider( "option", "max" );
        var items_count_med = 0;
        
        $('#med_values_phen_right :selected').each((key, el) => {
            var txt = $(el).val();
            $("#med_values_phen_right option[value='" + txt + "']").remove();
            items_count_med++;
        });
        $('#med_values_phen_right').filterByText($('#search_med_phen_right'));
       
        $("#similarity-range-meds").slider( "option", "max", max_val_meds - items_count_med );

        if (max_val_meds - items_count_med > 0) {
            $("#similarity-range-meds").slider('enable');
        }
        else{
            $("#similarity-range-meds").slider( "option", "min", 0);
            $("#similarity-range-meds").slider( "option", "max", 0);
            $("#similarity-range-meds").slider( "option", "value", 1);
        }
        if ($("#similarity-range-meds").slider('values', 0) != 0) {
            $("#sr-handle-meds").text($("#similarity-range-meds").slider('values', 0));
        }
        else{
            $("#sr-handle-meds").text('');
        }
    ...
    $('#btnClr').click(function(){
            $("#values_phen_right option").each(function(){
                $("#values_phen_right option").remove();
            });
            $("#similarity-range").slider( "option", "min", 0);
            $("#similarity-range").slider( "option", "value", 1);
            $("#similarity-range").slider( "option", "max", 1);
            $("#sr-handle").text('0');
            $("#similarity-range").slider('disable');
    });
    $('#btnClrMed').click(function(){
        $("#med_values_phen_right option").each(function(){
            $("#med_values_phen_right option").remove();
        });
        $("#similarity-range-meds").slider( "option", "min", 0);
        $("#similarity-range-meds").slider( "option", "value", 1);
        $("#similarity-range-meds").slider( "option", "max", 1);
        $("#sr-handle-meds").text('0');
        $("#similarity-range-meds").slider('disable');
        $("#similarity-range").slider( "option", "max" );
    });
    $('button.btnAdd').click(() => {
        $("#similarity-range").slider('disable');
    ...
    $('button.btnAdd2').click(() => {
        $("#similarity-range-meds").slider('disable');
        var max_val_meds = $("#similarity-range-meds").slider( "option", "max" );
        var items_count_med =0;
        $('#med_values_phen_left :selected').each((key, el) => {
            var txt = $(el).text();
            if ($("#med_values_phen_right option[value='" + txt + "']").length == 0) {
                $('#med_values_phen_right').append($("<option></option>").attr("value", txt).text(txt));    
                items_count_med++;
            }
        });
        $('#med_values_phen_right').filterByText($('#search_med_phen_right'));

        if ((max_val_meds + items_count_med) == 1) {
            $("#similarity-range-meds").slider( "option", "min", 0);
            $("#similarity-range-meds").slider( "option", "value", 1);
            $("#similarity-range-meds").slider( "option", "max", items_count_med + max_val_meds);
        }
        else if (max_val_meds ==  $("#similarity-range-meds").slider( "option", "value")) {
            $("#similarity-range-meds").slider( "option", "max", items_count_med + max_val_meds);
            $("#similarity-range-meds").slider( "option", "min", 1);
            $("#similarity-range-meds").slider( "option", "value", items_count_med + max_val_meds);
            $("#similarity-range-meds").slider('enable');
        }
        else{
            $("#similarity-range-meds").slider( "option", "min", 1);
    ...
    // $('ul#select2-medCodeMaySelect-container.select2-selection__rendered').text().split('×')
    $('#sizespec').click(()=>{
    // $('ul#select2-medCodeMaySelect-container.select2-selection__rendered').click(()=>{
        $("#similarity-range-maybe").slider('disable');
        var max_val_maybe = $("#similarity-range-maybe").slider( "option", "max" );
        var items_count_maybe =0;
        $('#medCodeMaySelect :selected').each((key, el) => {
            items_count_maybe++;
        });

        if ((max_val_maybe + items_count_maybe) == 1) {
            $("#similarity-range-maybe").slider( "option", "min", 0);
            $("#similarity-range-maybe").slider( "option", "value", 1);
            $("#similarity-range-maybe").slider( "option", "max", items_count_maybe + max_val_maybe);
        }
        else if (max_val_maybe ==  $("#similarity-range-maybe").slider( "option", "value")) {
            $("#similarity-range-maybe").slider( "option", "max", items_count_maybe + max_val_maybe);
            $("#similarity-range-maybe").slider( "option", "min", 1);
            $("#similarity-range-maybe").slider( "option", "value", items_count_maybe + max_val_maybe);
            $("#similarity-range-maybe").slider('enable');
        }
        else{
            $("#similarity-range-maybe").slider( "option", "min", 1);
            $("#similarity-range-maybe").slider( "option", "max", items_count_maybe + max_val_maybe);
        ...
        $( "#sr-handle-maybe" ).text($("#similarity-range-maybe").slider('values', 0));
    });
    ...
    $('#build_query').click(() => {
        ...
        .done((jsonAPI) => {
            ...
            if (min_diag_age > abs_min_diag_age) {
                logic_eav(['age', '>=', min_diag_age], eav, logic);
            }
            if (max_diag_age < abs_max_diag_age) {
                logic_eav(['age', '<=', max_diag_age], eav, logic);
            }
            ...
            if (!$('#genany').prop('checked')) {
                ...
                // else if ($('#genunk').prop('checked')) {
                //     logic_eav(["Gender", "is", "Unk"], eav, logic);
                // }
            }

            if (!$('#ddiagany').prop('checked')) {
                if ($('#ddiagyes').prop('checked')) {
                    logic_eav(["diagnosis", "is", "present"], eav, logic);
                }
                else if ($('#ddiagno').prop('checked')) {
                    logic_eav(["diagnosis", "is", "absent"], eav, logic);
                }
            }

            if (!$('#prescany').prop('checked')) {
                if ($('#prescyes').prop('checked')) {
                    logic_eav(["prescription", "is", "present"], eav, logic);
                }
                else if ($('#prescnono').prop('checked')) {
                    logic_eav(["prescription", "is", "absent"], eav, logic);
                }
            }

            // var diagDate = $('#snomedDate').datepicker("getDate");
            var diagDate = $('#snomedDate').val();
            logic_eav(['dob','is',diagDate],eav,logic);
            // var medDate = $('#medDate').datepicker("getDate");
            var medDate = $('#medDate').val();
            logic_eav(['med_date','is',medDate],eav,logic);

            var diag_date_selector = $('#diagDateSelect :selected').val();
            if (diag_date_selector != "any") {
                logic_eav(["diag_date_range", "is", diag_date_selector], eav, logic);
            }

            var presc_date_selector = $('#prescribedDateSelect :selected').val();
            if (presc_date_selector != "any") {
                logic_eav(["presc_date_range", "is", presc_date_selector], eav, logic);
            }
       ...
       // if(phenLogic === 'SIM' && $("#values_phen_right option").length > 0) {
            ...
            //             'ids': terms
            //         }
            //             // logic_eav(["term1", "is", terms[0]], eav, logic); // || term2 || term3 || term4 || term5 || term6 ...
            ...
            //         if(phenLogic === 'AND') {
            //             phe.push({'attribute' : "term1", 'operator': "is", 'value': term})
            ...
            // logic_eavt = [];
            // eavt = [];
            // if(eavlogic === 'EAV' && $("#values_phen_right option").length > 0) {
            //     termss=[];
            //     $("#values_phen_right option").each(function(){ termss.push($(this).val().split(' ')[0].replace(/[()]/g, ''))})
            //     eavt[0]={'attribute':'term1','operator':'is','value':termss}
            //     logic['-AND'].push('/query/components/eav/0')
            // }
            // else{
            //     $("#values_phen_right option").each(function(){
            //         var termt = $(this).val().split(' ')[0].replace(/[()]/g, '')

            //         if(eavlogic === 'AND'){
            //             eav.push({'attribute' : "term1", 'operator': "is", 'value': termt})
            //             logic['-AND'].push("/query/components/eav/" + (eav.length-1))
            //         } else {
            //             eav.push({'attribute' : "term1", 'operator': "is", 'value': termt})
            //             logic_eavt.push("/query/components/eav/" + (eav.length-1))
            //         }
            //     });
            //     if(logic_eavt.length > 1 && eavlogic === 'OR') {logic['-AND'].push({'-OR': logic_eavt})}    
            // }

            // if($("#values_phen_right option").length = 1){ //single snomed term
            //     terms = [];
            //     $("#values_phen_right option").each(function() { terms.push($(this).val().split(' ')[0].replace(/[()]/g, ''))})
            //     console.log(terms);
            //     logic_eav(["term1", "is", terms[0]], eav, logic); // || term2 || term3 || term4 || term5 || term6 ...
            ...
            if ($("#values_phen_right option").length >= 1) { // for multiple snomed terms
                // .each(function(){$(this).val()}).
                snomed_terms = [];
                i=1;
                $("#values_phen_right option").each(function() { snomed_terms.push($(this).val().split(' ')[0].replace(/[()]/g, ''))})
                
                $(snomed_terms).each(function(){
                    console.log(snomed_terms[i-1]);
                    logic_eav(["term"+(i),"is",snomed_terms[i-1]],eav,logic); // || term2 || term3 || term4 || term5 || term6 ...
                    i++;
                })
            }

            if($("#med_values_phen_right option").length >= 1){
                terms = [];
                i=1;
                // $("#med_values_phen_right option").each(function() { terms.push($(this).val().split(' (')[1].replace(/[()]/g, ''))})
                // console.log(terms[0]);
                // logic_eav(["medicationname", "is", terms[0]], eav, logic);
                $("#med_values_phen_right option").each(function() { terms.push($(this).val().split(' ')[0].replace(/[()]/g, ''))})
                $(terms).each(function(){
                    console.log(terms[i-1]);
                    logic_eav(["medterm"+(i),"is",terms[i-1]],eav,logic); // || term2 || term3 || term4 || term5 || term6 ...
                    i++;
                })
                // console.log(terms[0]);
                // logic_eav(["medterm0", "is", terms[0]], eav, logic);
            }
